...
We will see a quick setup to configure the ``translate`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``translate`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``translate`` formulas under ``/srv/salt/translate/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/translate/parameters/dns:domain/
    ln -s dns:domain /srv/salt/translate/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/translate/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/translate/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``translate`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``translate`` formulas under ``/srv/salt/translate/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/translate/parameters/roles
...
We create a configuration for the role ``translate/server`` in ``/srv/salt/translate/parameters/roles/translate/server.yaml``:
...
We create another configuration for the role ``translate/client`` in ``/srv/salt/translate/parameters/roles/translate/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles translate/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles translate/client
...
.. code-block:: console
    ...
    mv /srv/salt/translate/parameters/dns:domain/  '/srv/salt/translate/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-translate-as-json:
      test.nop:
        - name: {{ translate | json }}
...
Here is an example based on `template-formula/translate/config/file.sls`_:
    ...
    translate-config-file-file-managed:
      file.managed:
        ...
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='translate-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ translate.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``translate`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ translate['winner'] }}
...
.. _template-formula/translate/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/translate/config/file.sls
